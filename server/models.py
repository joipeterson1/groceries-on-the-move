...
# Cart model: A cart is linked to a customer, and contains multiple cart items
class Cart(db.Model, SerializerMixin):
    __tablename__ = 'carts'

    serialize_rules = ('-customer.cart', '-cart_items.cart', '-cart_items.product.orders')
    ...
    def to_dict(self):
        return {
            ...
            "cart_items": [item.to_dict() for item in self.cart_items]  # Convert cart items to dict
        }

# CartItem model: Represents a product in the cart, with a quantity
class CartItem(db.Model, SerializerMixin):
    __tablename__ = 'cart_items'

    serialize_rules = ('-cart.cart_items.cart', '-product.orders',)
    ...
    def to_dict(self):
        return {
            ...
            "product_img": self.product.product_img
        }

# Add a back reference in the Product model to CartItem
Product.cart_items = db.relationship('CartItem', back_populates='product', lazy='dynamic')

# Add a back reference in the Customer model to Cart
Customer.cart = db.relationship('Cart', back_populates='customer', uselist=False)  # One-to-one relationship with cart
